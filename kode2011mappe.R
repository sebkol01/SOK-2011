...
labs <- c("Gjennomsnitlig aarlig vekstrate i BNP pc 2000-2019 (%)", "Gjennomsnittlig aarlig befolkningsvekst (%)", "Gjennomsnittlig utdanningsnivaa", "Gjennomsnittlig netto sparing", "BNP per innbygger", "Humankapital/Gjennomsnittlig år i skole", "Gjennomsnittlig årlig vekstrate i eksport", "Gjennomsnittlig negativ vekstrate i naturresurser", "Gjennomsnittlig årlig vekstrate i investeringer", "Gjennomsnittlig årlig vekstrate i arbeidskraft") 
...
plot2 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = ln_gdppc, na.rm = TRUE)) +
  ...
  scale_x_continuous() # Velg vilke "ticks" som skal vises på x-akselen
plot2
...
model2 <- lm(ln_gdppc  ~ avg_educ + avg_nsy + avg_p + avg_nry + avg_n + avg_gi + avg_gx + gdppc0, data= no_outliers)
tab_model(model2)
summary(model2)

# Definer produksjonsfunksjonen
F <- function(K, L) {
  A <- 1  # teknologisk nivå (kan endres ved behov)
  alpha <- 0.3  # kapitalandel (kan endres ved behov)
...
# Sett opp dataene
k <- c(1, 2, 3, 4)
y <- c(1.00, 1.32, 1.55, 1.74)
v <- k/y

# Lag tabellen
data <- data.frame(k, y, v)

# Plot produksjonsfunksjonen og avtakende grenseproduktivitet
K <- seq(0, 5, by = 0.1)
L <- rep(1, length(K))
Y <- F(K, L)
MPK <- alpha * F(K, L) / K  # grenseproduktivitet av kapital
plot(K, Y, type = "l", xlab = "Kapital (K)", ylab = "Produksjon (y)",
     main = "Avtakende grenseproduktivitet")